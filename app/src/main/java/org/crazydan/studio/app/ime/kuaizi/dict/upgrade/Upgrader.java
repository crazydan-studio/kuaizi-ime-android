...
public abstract class Upgrader {
    ...
    public abstract void upgrade(Context context, IMEditorDict dict);
    ...
    static class DBFiles {
        /** {@link DictDBType#user 用户库}对应的数据库文件 */
        protected File user;

        /**
         * 数据迁移库
         * <p/>
         * 在需要将{@link DictDBType#user 用户库}和{@link DictDBType#app_word 字典库}进行合并时，
         * 需通过该迁移库作为中间库来实施数据迁移和合并，从而避免迁移失败而导致用户数据丢失
         * <p/>
         * 该迁移库实际为应用安装包内{@link DictDBType#app_word 字典库}的副本，可以对其进行反复重试，
         * 在操作全部成功后，需用该文件替换 {@link #user} 库文件，
         * 该替换操作会在函数 {@link #doWithTransferDB} 内自动完成
         */
        protected File dataTransfer;

        /** 应用安装包内{@link DictDBType#app_phrase 词典库}的副本 */
     ...
     */
    static void doWithTransferDB(Context context, IMEditorDict dict, DBTransferConsumer consumer) {
        File appPhraseDBFile = dict.getDBFile(context, DictDBType.app_phrase);

        File userDBFile = dict.getUserDBFile(context);
        // 使用应用的字典库作为迁移库
        File transferDBFile = dict.getDBFile(context, DictDBType.app_word);
