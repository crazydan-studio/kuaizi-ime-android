...
| `View` | 叶子视图（独立的可渲染单元） | `XPadView`, `KeyView` |
| `Bridge` | 桥梁实现（对接引擎输出到编辑器） | `TextEditorBridge` |
...
| `ImeSupportEditText` / `ImeEditText` / `EditorField` | `TextEditorBridge` | 桥梁模式替代独立编辑框组件 |
...
- `CandidateBar` / `InputBar`（使用 `CandidatePanel` / `InputListPanel`）
...
- `EditorField`（使用 `TextEditorBridge`）
- `EditorHost`（使用 Bridge 接入示例，见文档 160 §5.5）
- `InputHostView`（使用 Bridge 接入示例，见文档 160 §5.5）
- `ImeEditText` / `ImeSupportEditText`（使用 `TextEditorBridge`）
