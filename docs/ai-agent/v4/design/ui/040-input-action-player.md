...
**本文档范围**：动画相关的数据模型（`InputActionPlaybackState`、`InputActionFingerIndicator`、`InputActionRowIndicator`、`InputActionPathInterpolator`、`InputActionPositionResolver` 接口）均定义在 [engine/060-input-action.md](../engine/060-input-action.md)，此处不再重复定义。本文档覆盖 UI 层的实现：`KeyboardInputActionPlayerHost`、`ComposeInputActionPositionResolver`、`InputActionPlayer` 的使用方式和行指示器内建机制。
...
`ComposeInputActionPositionResolver` 是 `InputActionPositionResolver` 接口（定义在 [engine/060-input-action.md](../engine/060-input-action.md)）的 Compose 布局实现，将语义按键标识符和列表索引解析为归一化坐标。该实现替代了 v4 的 `ComposeKeyPositionResolver`（返回绝对像素坐标），现在返回归一化坐标 [0,1]x[0,1]，由 GestureFeedbackPanel 根据面板尺寸反归一化后绘制。
 ...
 * 替代 v4 的 ComposeKeyPositionResolver（返回绝对像素坐标），
 * 现在返回归一化坐标 [0,1]x[0,1]。
 *
 * 三类布局状态提供者均为惰性求值：
...
| 所属包 | panel |
| 前身 | v4 的 ComposeKeyPositionResolver（返回绝对像素坐标） |

---

## 4. InputActionPlayer

`InputActionPlayer` 是输入动作播放引擎，接收坐标无关的 ActionScript，按时间轴依次执行动作。与 v4 的核心变更包括：构造参数从 `KeyPositionResolver` 改为 `InputActionPositionResolver`（返回归一化坐标），播放状态从 `PlaybackState` 改为 `InputActionPlaybackState`（定义在 engine/060），路径插值从 `SwipePathInterpolator` 改为 `InputActionPathInterpolator`（定义在 engine/060），新增行指示器状态管理。
 ...
 *
 * 与 v4 的核心变更：
 * - positionResolver 类型从 KeyPositionResolver 改为 InputActionPositionResolver
 * - playbackState 类型从 PlaybackState 改为 InputActionPlaybackState
 * - 路径插值从 SwipePathInterpolator 改为 InputActionPathInterpolator
 * - 所有坐标均为归一化坐标 [0,1]x[0,1]
 * - 新增 row1IndicatorState 和 row2IndicatorState 管理行指示器
 */
class InputActionPlayer(
    ...
    private val _row1IndicatorState = MutableStateFlow<InputActionRowIndicator?>(null)
    val row1IndicatorState: StateFlow<InputActionRowIndicator?> = _row1IndicatorState.asStateFlow()

    // Row 2 行指示器状态（归一化坐标，行相对）
    private val _row2IndicatorState = MutableStateFlow<InputActionRowIndicator?>(null)
    val row2IndicatorState: StateFlow<InputActionRowIndicator?> = _row2IndicatorState.asStateFlow()
    ...
    private fun executeAction(action: InputAction) {
        when (action) {
            ...
            is InputAction.SelectCandidate -> {
                ...
                // 更新 Row 1 行指示器
                _row1IndicatorState.value = InputActionRowIndicator(
...
|------|------|
| 角色 | 输入动作播放引擎，坐标无关 |
| 职责 | 按 ActionScript 时间轴执行 InputAction，驱动 GestureFeedbackState 和 KeyboardViewModel |
...
| 行指示器 | row1IndicatorState: MutableStateFlow\<InputActionRowIndicator?\>, row2IndicatorState: MutableStateFlow\<InputActionRowIndicator?\> |
...
| 所属包 | player |
| 前身 | v4 的 InputActionPlayer（使用 KeyPositionResolver + PlaybackState + SwipePathInterpolator + 绝对坐标） |
...
| 行 | 面板 | 指示器方式 | 参数 |
|---|---|---|---|
| Row 1 | CandidateListPanel | 面板内建 Canvas 绘制 | showIndicator: Boolean, indicatorState: InputActionRowIndicator? |
| Row 2 | InputListPanel / ToolListPanel | 面板内建 Canvas 绘制 | showIndicator: Boolean, indicatorState: InputActionRowIndicator? |
...
### 5.4 InputActionRowIndicator 模型

`InputActionRowIndicator` 是行指示器状态的数据模型，使用归一化坐标（行相对），定义在 [engine/060-input-action.md](../engine/060-input-action.md)：
...
| 所属模块 | :ime-engine（定义），:ime-ui（使用） |

---

## 6. 播放执行流程

输入动作播放的执行流程如下。播放器加载 ActionScript 后，按时间轴依次执行 InputAction。对于不同类型的动作，播放器通过 `InputActionPositionResolver` 解析归一化坐标，通过 `InputActionPathInterpolator` 生成插值轨迹，更新 `GestureFeedbackState` 的手指指示器和触摸轨迹，同时管理行指示器状态。
...
**SelectCandidate 执行**：播放器通过 `resolveCandidatePosition(index)` 获取候选项的归一化坐标（行相对），更新 `row1IndicatorState` 为 `InputActionRowIndicator`。CandidateListPanel 在内部检测到 `showIndicator=true` 且 `indicatorState` 非空后，根据面板尺寸反归一化绘制圆形指示器。短暂延时后清除指示器。

**行指示器与 FingerIndicator 的分工**：Row 1 和 Row 2 的点击类操作（SelectCandidate、SelectInputItem）使用行指示器（`InputActionRowIndicator`），在面板内部绘制；Row 3 的按键操作（KeyDown、SwipeTo、KeyUp）使用 FingerIndicator（`InputActionFingerIndicator`），通过 GestureFeedbackPanel 绘制。两者均使用归一化坐标，但坐标系不同：行指示器使用行相对坐标系，FingerIndicator 使用 KeyLayoutPanel 的归一化坐标系。

---

## 7. ActionScriptLoader

`ActionScriptLoader` 负责从文件系统加载动作脚本，与 v4 的核心逻辑相同，但更新了类型引用以匹配新的模型命名。

```kotlin
/**
 * 动作脚本加载器。
 *
 * 从 assets 或外部文件加载 ActionScript，
 * 使用 Kotlin Serialization 解析 JSON 格式的脚本。
 * 脚本格式定义在 engine/060-input-action.md。
 */
...
|------|------|
| 角色 | 动作脚本文件加载器 |
| 职责 | 从 assets 或外部 URI 加载 JSON 格式的 ActionScript |
| 约束 | 仅负责加载和反序列化，不执行脚本 |
...
InputActionPlayer.executeAction(InputAction)
  ...
  |     | InputActionRowIndicator (归一化坐标，行相对)
  |     v
...
3. **写入阶段**：归一化坐标写入 `GestureFeedbackState`（FingerIndicator、touchTrailPoints）或 `InputActionRowIndicator`（行指示器）。
4. **绘制阶段**：GestureFeedbackPanel 读取归一化坐标后根据面板尺寸反归一化绘制；面板内建指示器读取行相对归一化坐标后根据面板尺寸反归一化绘制。

```plantuml
@file:../diagrams/ui-input-action-normalized-coordinate-flow.puml
```
