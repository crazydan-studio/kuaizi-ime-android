...
InputMode 决定交互范式和布局几何，是独立于键盘内容类型的正交维度。X-Pad 模式采用六边形网格布局，手指在六边形区域间滑行选择声母韵母组合；HexGrid 模式采用六边形网格但交互方式不同于 X-Pad，适用于需要六边形紧密排列的场景；RectGrid 模式采用传统矩形网格布局，是最常见的 QWERTY 式按键排列；MultiZone 模式将键盘划分为多个独立区域，每个区域可独立交互，适用于需要分区操作的场景。InputMode 的选择直接影响 KeyLayoutPanel 的布局算法和 GestureInputPanel 的手势识别逻辑。
...
Keyboard.Type 决定键盘的内容类型，即按键集合的语义分类。Pinyin 类型提供拼音输入的声母韵母按键，Latin 类型提供拉丁字母按键，Symbol/Emoji 类型提供符号和表情，Number 类型提供数字和基本运算符，Math 类型提供数学公式相关按键，Editor 类型提供文本编辑功能键。Type 的选择决定了 KeyLayoutPanel 渲染哪些按键以及按键的标签内容，但不影响按键的几何排列方式——几何排列由 InputMode 决定。
...
|  | Pinyin | Latin | Symbol/Emoji | Number | Math | Editor |
|---|---|---|---|---|---|---|
| **XPad** | 六边形拼音滑行 | 六边形拉丁滑行 | 六边形符号选择 | 六边形数字输入 | 六边形公式输入 | 六边形编辑操作 |
| **HexGrid** | 六边形拼音网格 | 六边形拉丁网格 | 六边形符号网格 | 六边形数字网格 | 六边形公式网格 | 六边形编辑网格 |
| **RectGrid** | QWERTY 拼音键盘 | QWERTY 拉丁键盘 | 矩形符号键盘 | 矩形数字键盘 | 矩形公式键盘 | 矩形编辑键盘 |
| **MultiZone** | 分区拼音输入 | 分区拉丁输入 | 分区符号输入 | 分区数字输入 | 分区公式输入 | 分区编辑操作 |
...
三行结构的设计考量在于：将不同交互职责的面板在空间上分离，避免面板之间的视觉干扰和事件冲突。Row 1 的候选提示和操作提示在功能上相近（都是展示信息），共享空间但通过叠加方式共存；Row 2 的工具栏和输入栏在时间上互斥（空闲时用工具、输入时用输入栏），共享空间通过切换方式共存；Row 3 的三层面板在交互上互补（触摸/反馈/渲染），必须始终共存通过叠加方式共享空间。
...
Zone A 仅在 Separated 模式下被使用，其内容由 KeyLayoutPanel 和 GestureFeedbackPanel 叠加构成。KeyLayoutPanel 负责根据当前 InputMode 和 Keyboard.Type 渲染按键布局，GestureFeedbackPanel 负责绘制手势轨迹、按键高亮、按键间路径等视觉反馈。两面板叠加共享 Zone A 的全部空间，叠加顺序从底到顶为：KeyLayoutPanel（底层渲染）-> GestureFeedbackPanel（透明反馈层）。Zone A 的容器支持指定尺寸和布局方向（居中、顶部对齐、底部对齐），面板组件自动填充容器提供的可用空间。
...
以下为 kuaizi IME UI 组件的完整层次树，从顶层集成组件到面板组件再到原子组件，按照组合关系组织。KeyboardHost 作为顶层集成组件，内部根据 LayoutMode 组合 Zone A 和 Zone B 的内容；Zone B 内部按三行结构组织面板；面板组件内部组合原子组件。KeyboardInputActionPlayerHost 是独立的演示组件，内部组合 KeyboardHost 和播放引擎。
...
├── Animation 模式:
│   ├── KeyboardHost
│   └── IndicatorOverlay x 3 (Row 1, Row 2, Row 3)
└── DirectInput 模式:
    ├── KeyboardHost (封装)
    └── IndicatorOverlay x 3
```
...
KeyboardHost 替代了 v4 中的 KeyboardPanel 和 KeyboardScreen 两个组件，通过 LayoutMode 参数统一了堆叠和分离两种布局模式的入口。这种统一简化了第三方应用的集成方式——只需使用 KeyboardHost 并指定 LayoutMode 即可获得完整的输入法界面，无需根据模式选择不同的组件。KeyboardInputActionPlayerHost 在 KeyboardHost 基础上叠加播放引擎和 IndicatorOverlay，专用于输入动作的演示和练习场景，不影响 KeyboardHost 的正常使用。
...
fun KeyboardHost(
    ...
    var keyPanelLayout by remember { mutableStateOf(KeyLayoutPanelLayoutInfo()) }

    KeyboardTheme(themeType = state.config.ui.themeType) {
        when (layoutMode) {
            is LayoutMode.Stacked -> StackedLayout(viewModel, state, feedbackState, keyPanelLayout)
            is LayoutMode.Separated -> SeparatedLayout(
                viewModel, state, feedbackState, keyPanelLayout, layoutMode as LayoutMode.Separated
            )
        }
    }
}
...
private fun StackedLayout(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        Box(modifier = Modifier.weight(RowWeight.R1)) {
            ...
            PopupTipPanel(feedbackState = feedbackState)
        }
...
|------|------|
| 角色 | Row 1 面板，展示候选列表 |
| 职责 | 展示多个可补全输入、可粘贴内容等候选项，支持滚动和选择 |
| 约束 | 始终部署在 Zone B Row 1，与 PopupTipPanel 叠加共享空间 |
| 关键属性 | state: CandidateListState, onCandidateSelected: (Candidate) -> Unit |
| 所属包 | candidate |
 ...
 * 候选列表面板布局信息，用于坐标解析。
 */
data class CandidateListLayoutInfo(
    /** 各候选项的位置矩形（屏幕坐标系） */
 ...
 */
fun CandidateListLayoutInfo.locateItem(index: Int): Offset? {
...
| 关键属性 | feedbackState: GestureFeedbackState |
| 所属包 | panel |
...
fun PopupTipPanel(
    ...
    val tipState by feedbackState.popupTip.collectAsState()
...
| 属性 | 说明 |
|------|------|
| 角色 | Row 2 面板（空闲时），展示工具按钮 |
| 职责 | 空闲时展示固定 ToolItem 按钮（剪贴板、收藏、设置等），输入时仅显示切换按钮 |
| 约束 | 与 InputListPanel 互斥共享 Row 2 空间，由 isInputting 状态控制切换 |
| 关键属性 | state: ToolListState, onToolSelected: (ImeIntent) -> Unit |
| 所属包 | panel |
...
| 职责 | 展示当前输入内容，支持选择已输入文本进行修改或选择候选 |
| 约束 | 与 ToolListPanel 互斥共享 Row 2 空间；提供 locateItem() 方法供播放器定位 |
| 关键属性 | state: InputListState, locateItem(): Offset? |
| 所属包 | input |
 ...
 * 输入列表面板布局信息。
 */
data class InputListLayoutInfo(
    /** 各输入项的位置矩形（屏幕坐标系） */
 ...
 */
fun InputListLayoutInfo.locateItem(index: Int): Offset? {
...
|------|------|
| 角色 | Row 3 面板，按键布局渲染 |
| 职责 | 根据 InputMode 和 Keyboard.Type 动态切换按键布局并渲染按键，提供布局信息供其他面板查询 |
...
| 所属包 | panel |
| 变更说明 | 由 v4 的 KeyGridPanel 更名，强调布局渲染职责而非仅网格渲染 |
...
/**
 * 按键布局面板布局信息，替代 v4 的 KeyGridPanelLayoutInfo。
 *
 ...
 * 绘制时根据面板实际尺寸转换为像素坐标。
 */
data class KeyLayoutPanelLayoutInfo(
    /** 按键归一化位置映射（归一化坐标） */
...
| 关键属性 | keyPanelLayout, inputMode, keyboardType, feedbackState, onGesture |
| 所属包 | panel |
 ...
 * 其触摸点通过 keyPanelLayout 映射到 Zone A 的 KeyLayoutPanel 进行按键定位。
 */
@Composable
...
|------|------|
| 角色 | 透明反馈绘制层，双实例 |
| 职责 | 根据归一化坐标绘制手势轨迹、按键高亮、按键间路径、X-Pad 路径高亮、手指指示器 |
| 约束 | 双实例：Zone A 实例和 Zone B 实例；使用归一化坐标，绘制时根据面板尺寸转换为像素坐标 |
| 关键属性 | elements: Set\<FeedbackElementType\>, feedbackState, keyPanelLayout |
| 所属包 | panel |
 ...
 * - Separated 模式：Zone A 实例绘制按键侧反馈（KeyHighlight, KeyPath, XPadPathHighlight），
 *   Zone B 实例绘制输入侧反馈（TouchTrail, FingerIndicator）
 *
 * @param elements 该实例绘制的反馈元素类型集合
 * @param feedbackState 手势反馈状态（归一化坐标）
 * @param keyPanelLayout 按键布局信息（归一化坐标），用于按键定位
 */
@Composable
fun GestureFeedbackPanel(
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        ...
        if (FeedbackElementType.KeyHighlight in elements && pressedKeys.isNotEmpty()) {
            drawKeyHighlights(pressedKeys, keyPanelLayout, panelSize)
        }

        // 按键间路径
        if (FeedbackElementType.KeyPath in elements && keyPath.size >= 2) {
            drawKeyPath(keyPath, keyPanelLayout, panelSize)
        }

        // X-Pad 路径高亮
        if (FeedbackElementType.XPadPathHighlight in elements && xPadPath.isNotEmpty()) {
            drawXPadPathHighlight(xPadPath, keyPanelLayout, panelSize)
        }
 ...
 * 反馈面板配置集。
 *
 * 替代 v4 的 GestureFeedbackPanelSet，更新命名。
 */
...
| 约束 | 仅用于演示/练习场景；Animation 模式访问真实字典数据但不提交到目标编辑器 |
| 关键属性 | useMode: UseMode, viewModel: KeyboardViewModel, IndicatorOverlay x 3 |
| 所属包 | integration |
 ...
 * 在面板上绘制圆形指示器点击动画，点击坐标由面板的 locateItem() 方法动态计算。
    ...
    Box(modifier = modifier.fillMaxSize()) {
        ...
        when (useMode) {
            is KeyboardInputActionPlayerHost.UseMode.Animation -> {
                // Animation 模式：使用引擎但不提交到编辑器
                KeyboardHost(viewModel = viewModel)
            }
            is KeyboardInputActionPlayerHost.UseMode.DirectInput -> {
                // DirectInput 模式：完整输入支持
                KeyboardHost(viewModel = viewModel)
            }
        }

        // IndicatorOverlay：三行各一个
        if (playerState is PlaybackState.Playing) {
            // Row 3 的指示器直接绘制在 GestureFeedbackPanel 上
            // Row 1 和 Row 2 各需独立覆盖层
            Row1IndicatorOverlay(viewModel.feedbackState)
            Row2IndicatorOverlay(viewModel.feedbackState)
        }
    }
}
...
叠加共存与互斥共存的核心区别在于：叠加共存的面板同时渲染在同一空间，通过透明度和 Z 轴顺序实现视觉分层；互斥共存的面板同一时刻仅渲染其中一个，切换时存在短暂的进入/退出动画。Row 1 的叠加方式允许 PopupTipPanel 在不影响 CandidateListPanel 布局的情况下短暂浮现，提示消失后候选列表自然可见。Row 2 的互斥方式确保工具栏和输入栏不会视觉冲突，用户在输入时看到输入内容，空闲时看到工具选项。Row 3 的叠加方式延续了 v4 的三层分离设计，确保触摸/反馈/渲染三层的独立性。
...
ImeState 需要扩展以支持新的 UI 层概念。新增 inputMode 字段表示当前输入模式，isInputting 字段表示是否正在输入（控制 ToolListPanel 和 InputListPanel 的互斥切换），以及 Zone 相关的布局配置。这些扩展仅涉及 UI 层状态的暴露，不改变 `:ime-engine` 的核心 reduce 逻辑——引擎仍然通过 ImeIntent 驱动状态转换，UI 层从 ImeState 中读取新增字段来决定面板的部署和切换。
...
### 8.2 GestureFeedbackState 归一化

GestureFeedbackState 需要从绝对坐标改为归一化坐标存储，这是支持双实例 GestureFeedbackPanel 的关键变更。在 v4 中，GestureFeedbackState 存储的是绝对像素坐标，仅在单实例反馈面板中使用时没有问题；但在分离模式下，Zone A 和 Zone B 的反馈面板尺寸不同，同一份坐标数据无法在两个面板上正确渲染。归一化坐标的引入解决了这个问题：GestureFeedbackState 统一存储归一化坐标，各反馈面板实例在绘制时根据自身尺寸将归一化坐标转换为像素坐标。

```kotlin
/**
 * 手势反馈状态，使用归一化坐标。
 *
 * 与 v4 的关键变更：所有坐标数据以归一化形式 [0,1]x[0,1] 存储，
...
class GestureFeedbackState {
     ...
     * 弹出提示状态。
     */
    private val _popupTip = MutableStateFlow<PopupTipState?>(null)
    val popupTip: StateFlow<PopupTipState?> = _popupTip.asStateFlow()
    ...
    fun setKeyPath(keys: List<InputKey>) { _keyPath.value = keys }
    fun clearKeyPath() { _keyPath.value = emptyList() }
    fun setXPadPath(zones: List<XPadZone>) { _xPadPath.value = zones }
    fun clearXPadPath() { _xPadPath.value = emptyList() }
    fun setFingerIndicator(state: FingerIndicatorState?) { _fingerIndicator.value = state }
    fun showPopupTip(message: String) {
        _popupTip.value = PopupTipState(message)
    }
...
KeyboardViewModel 需要扩展以支持 LayoutMode 管理、归一化坐标转换和 InputActionPlayer 集成。新增 layoutMode 状态管理，支持运行时动态切换；新增 actionPlayer 实例，提供输入动作播放能力；手势反馈状态中的坐标归一化逻辑集成到 ViewModel 中，确保输入面板写入的归一化坐标与反馈面板读取的归一化坐标一致。
...
class KeyboardViewModel(
    ...
    val actionPlayer = InputActionPlayer(
        viewModel = this,
        feedbackState = feedbackState,
        positionResolver = ComposeKeyPositionResolver(
            keyboardLayoutProvider = { _currentKeyLayout },
            candidateLayoutProvider = { _currentCandidateLayout },
            inputListLayoutProvider = { _currentInputListLayout },
        ),
    ...
    private var _currentKeyLayout: KeyLayoutPanelLayoutInfo? = null
    private var _currentCandidateLayout: CandidateListLayoutInfo? = null
    private var _currentInputListLayout: InputListLayoutInfo? = null

    fun updateKeyLayoutInfo(info: KeyLayoutPanelLayoutInfo) {
        _currentKeyLayout = info
    }

    fun updateCandidateLayoutInfo(info: CandidateListLayoutInfo) {
        _currentCandidateLayout = info
    }

    fun updateInputListLayoutInfo(info: InputListLayoutInfo) {
        _currentInputListLayout = info
    }
...
以下展示用户手势输入和程序化输入两条数据流，标注归一化坐标的转换节点。
...
GestureInputPanel
  | 查询 KeyLayoutPanelLayoutInfo.findKeyAt()
  | 输出 InputGesture
  v
...
ImeState
  | collectAsState()
  v
KeyLayoutPanel / CandidateListPanel / InputListPanel / ToolListPanel
```
...
InputActionPlayer.executeAction(InputAction)
  | 查询 KeyPositionResolver.resolve(key)
  | KeyLayoutPanel 动态计算归一化坐标
  | 写入 GestureFeedbackState (归一化坐标)
  v
  +---> GestureFeedbackState
  |     | fingerIndicator: OffsetF
  |     | pressedKeys: Set<InputKey>
  |     v
...
KeyboardInputActionPlayerHost 是输入动作播放的集成组件，内部组合 KeyboardHost 和播放引擎，通过 IndicatorOverlay 在 Zone B 的三行上绘制指示器动画。播放引擎沿用 [040-input-action-player.md](040-input-action-player.md) 中的 InputActionPlayer 设计，但坐标体系从绝对坐标改为归一化坐标，KeyPositionResolver 从解析绝对像素坐标改为解析归一化坐标。ActionScript 和 InputAction 沿用 [engine/060-input-action.md](../engine/060-input-action.md) 的定义，保持坐标无关设计。

播放引擎的工作方式取决于 UseMode。Animation 模式下，播放器访问真实字典数据（通过 ImeEngine 的正常 reduce 路径），但不将结果提交到目标编辑器，也不写入数据库——这意味着 ImeEngine 的状态机正常运转，UI 正常渲染键盘状态变化，但 ImeOutput 不会被分发。DirectInput 模式下，播放器在完整输入流程上叠加动画效果，结果会正常提交到目标编辑器，适用于实际输入辅助场景。

### 9.2 KeyPositionResolver 扩展

KeyPositionResolver 从解析绝对像素坐标扩展为解析归一化坐标，并新增 InputListPanel 和 CandidateListPanel 的定位支持。对于 KeyLayoutPanel 的按键定位，解析器查询 KeyLayoutPanelLayoutInfo 中的归一化位置映射；对于 CandidateListPanel 和 InputListPanel 的项定位，解析器调用面板的 locateItem() 方法获取像素坐标后再归一化。若目标项不在可视范围内，需先滚动到目标位置再定位。
...
class ComposeKeyPositionResolver(
    private val keyboardLayoutProvider: () -> KeyLayoutPanelLayoutInfo?,
    private val candidateLayoutProvider: () -> CandidateListLayoutInfo?,
    private val inputListLayoutProvider: () -> InputListLayoutInfo?,
) : KeyPositionResolver {
    ...
    override fun resolveInputItemPosition(index: Int): OffsetF? {
        val layout = inputListLayoutProvider() ?: return null
        val pixelOffset = layout.locateItem(index) ?: return null
...
### 9.3 IndicatorOverlay 机制

Zone B 三行结构中，每行都需要在播放动画时展示指示器。Row 3 的指示器可以直接绘制在 GestureFeedbackPanel 上，因为 GestureFeedbackPanel 已经是 Row 3 的透明覆盖层，只需在反馈面板的绘制逻辑中增加指示器类型即可。Row 1 和 Row 2 则需要独立的透明覆盖层（IndicatorOverlay），因为这两行没有类似 GestureFeedbackPanel 的现成透明层可供使用。IndicatorOverlay 是半透明的遮罩层，仅在播放动画期间启用，在面板上绘制圆形指示器，使用行相对坐标进行定位。
 ...
 *
 * 半透明遮罩层，仅在输入动作播放期间启用。
 * 在面板上绘制圆形指示器，使用行相对坐标定位。
 * 适用于 Zone B 的 Row 1 和 Row 2，
 * Row 3 的指示器直接绘制在 GestureFeedbackPanel 上。
 */
@Composable
fun Row1IndicatorOverlay(
    feedbackState: GestureFeedbackState,
    modifier: Modifier = Modifier,
) {
    val indicatorState by feedbackState.row1Indicator.collectAsState()

    if (indicatorState == null) return

    Canvas(modifier = modifier.fillMaxSize()) {
        indicatorState?.let { indicator ->
            val pixelPosition = indicator.position.denormalize(size)
            drawCircle(
...
@Composable
fun Row2IndicatorOverlay(
    feedbackState: GestureFeedbackState,
    modifier: Modifier = Modifier,
) {
    val indicatorState by feedbackState.row2Indicator.collectAsState()
...
输入动作播放的执行流程如下，与 [040-input-action-player.md](040-input-action-player.md) 中的流程类似，但增加了归一化坐标和 IndicatorOverlay 的处理。播放器加载 ActionScript 后，按时间轴依次执行 InputAction。对于 KeyDown/SwipeTo/KeyUp 等按键相关动作，播放器通过 KeyPositionResolver 解析归一化坐标，更新 GestureFeedbackState 的手指指示器和按键高亮，反馈面板据此绘制动画。对于 SelectCandidate 动作，播放器通过 KeyPositionResolver.resolveCandidatePosition() 获取候选项的归一化坐标，更新 Row 1 的 IndicatorOverlay，在候选项上方绘制圆形点击动画。对于 InputListPanel 相关交互，同理使用 resolveInputItemPosition() 和 Row 2 的 IndicatorOverlay。
...
遍历 ActionScript.actions:
  |
  +-- InputAction.KeyDown
  |     | KeyPositionResolver.resolve(key) -> OffsetF
  |     | feedbackState.setFingerIndicator(FingerIndicatorState(OffsetF, pressed=true))
  ...
  |     | 生成归一化坐标插值路径
  |     | animateFingerAlongPath(feedbackState, normalizedPath, duration)
  |     | viewModel.handleIntent(PressKey(toKey, Swipe))
  |     v
  |
  +-- InputAction.KeyUp
  |     | feedbackState.setFingerIndicator(FingerIndicatorState(position, pressed=false))
  |     v
  |
  +-- InputAction.SelectCandidate
  |     | resolveCandidatePosition(index) -> OffsetF (Row 1 归一化坐标)
  |     | feedbackState.setRow1Indicator(RowIndicatorState(OffsetF))
  |     | viewModel.handleIntent(SelectCandidate(...))
  |     | delay -> feedbackState.setRow1Indicator(null)
  |     v
  |
...
播放结束
  ...
  feedbackState.setRow1Indicator(null)
  feedbackState.setRow2Indicator(null)
```
...
以下设计是本次新增的概念和组件。Zone A / Zone B 屏幕分区模型是全新的空间组织概念，将 IME 屏幕纵向划分为两个功能区，使得分离布局模式下的空间分配有了清晰的语义定义。Zone B 三行结构是全新的面板组织方式，将候选、工具/输入、键盘交互三类面板在空间上分离，替代了 v4 中线性的 Column 布局。PopupTipPanel 是新增的短暂提示面板，替代了 v4 早期迭代中的 InfoTipPanel。ToolListPanel 是新增的工具栏面板，在空闲时展示功能按钮。KeyboardHost 是新增的统一集成组件，替代了 KeyboardPanel 和 KeyboardScreen 两个组件。KeyboardInputActionPlayerHost 是新增的播放演示集成组件，支持 Animation 和 DirectInput 两种使用模式。CoordinateNormalizer 是新增的坐标归一化工具，支持归一化坐标与绝对坐标的双向转换。IndicatorOverlay 是新增的行指示器覆盖层机制，在播放动画时在各行上绘制指示器。InputMode 枚举（XPad / HexGrid / RectGrid / MultiZone）是新增的交互范式维度，与 Keyboard.Type 正交组合。

### 10.3 修改的设计

以下设计从 v4 继承但做了重要修改。LayoutMode 命名从 Overlay/FullScreen 更新为 Stacked/Separated，语义更直观：Stacked 强调面板的堆叠共存，Separated 强调输入区域与按键区域的空间分离。KeyGridPanel 更名为 KeyLayoutPanel，强调其布局渲染职责而非仅网格渲染，并增加了 InputMode x Type 的正交组合支持。GestureFeedbackState 从绝对坐标存储改为归一化坐标存储，这是支持双实例 GestureFeedbackPanel 的关键变更。KeyPositionResolver 从解析绝对像素坐标改为解析归一化坐标，并新增 InputListPanel 和 CandidateListPanel 的定位方法。GestureFeedbackPanelSet 从 OverlaySet/FullScreenSet 更名为 StackedSet/SeparatedSet，与 LayoutMode 命名对齐。InputGesture 中增加了 InputMode 参数，使得手势识别逻辑可以根据不同输入模式采用不同策略。

### 10.4 废弃的设计

以下设计从 v4 中废弃。KeyboardPanel（原 Overlay 模式集成组件）被 KeyboardHost 替代，不再需要根据布局模式选择不同的集成组件。KeyboardScreen（原 FullScreen 模式集成组件）同样被 KeyboardHost 替代。InfoTipPanel（早期迭代的提示面板）被 PopupTipPanel 替代，强调了短暂弹出的交互特性。KeyGridPanelLayoutInfo 被 KeyLayoutPanelLayoutInfo 替代，使用归一化坐标替代绝对像素坐标。GestureFeedbackState 中的绝对坐标字段（`MutableStateFlow<List<Offset>>`）被归一化坐标字段（`MutableStateFlow<List<OffsetF>>`）替代。
...
| 提示面板 | InfoTipPanel | PopupTipPanel | 修改（重命名） |
| 工具面板 | Toolbar（线性排列） | ToolListPanel（互斥切换） | 新增 |
...
| 位置解析 | KeyPositionResolver (绝对坐标) | KeyPositionResolver (归一化坐标) | 修改 |
| InputMode | 无（隐含在 KeyboardType 中） | InputMode 枚举（正交于 Type） | 新增 |
| 指示器覆盖 | FingerOverlay / SwipeTrailOverlay / KeyHighlightOverlay | IndicatorOverlay + GestureFeedbackPanel 内置 | 修改 |
