...
| `ClipTipPopup` | `clipboard` | 剪贴板提示弹窗 |
| `FavoriteItem` | `favorite` | 收藏项（含滑动删除） |
| `FingerOverlay` | `practice` | 手指指示器（程序化输入动画） |
...
| `KeyGridPanel` / `StandardKeyGridPanel` | `panel` | 按键渲染层 |
| `GestureFeedbackPanel` | `panel` | 透明反馈绘制层 |
| `CandidateListPanel` | `candidate` | 候选栏 |
| `InputListPanel` | `input` | 输入栏 |
| `FavoriteListPanel` | `favorite` | 收藏面板 |
| `XPadView` | `keyboard` | X-Pad 六边形面板 |
| `ActionPlayerPanel` | `practice` | 播放控制面板 |
...
| 组件 | 包路径 | 说明 |
|------|--------|------|
| `KeyboardViewModel` | `viewmodel` | UI 层协调中心，持有 ImeEngine，暴露 `StateFlow<ImeState>`，将 InputGesture 转换为 ImeIntent，管理 GestureFeedbackState |
