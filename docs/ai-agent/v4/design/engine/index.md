...
本目录包含 `:engine` 引擎库的完整设计文档，按照业务分层和功能模块组织。引擎库提供核心 IME 引擎能力，独立于 UI 框架和平台。
...
| 020 | [全局状态模型](020-ime-state.md) | ImeState 完整 8 字段定义、Keyboard/CandidateList/Clipboard/FavoriteList/ToolListState 子状态、ImeEffect 副作用通道（PopupTip.Message/Action）、Feature 门控规则、状态不变式、状态频率分层 |
...
| 065 | [ImeEffect 副作用信号](065-ime-effect.md) | ImeEffect 密封类定义（PopupTip.Message/Action）、引擎领域事件发射时机（收藏确认、剪贴板提示、导入结果等） |
