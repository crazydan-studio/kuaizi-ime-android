# 160 — 三层模块划分：引擎库 `:ime-engine` + UI 库 `:ime-ui` + 应用模块 `:app`
...
| `:ime-ui` | 不使用 `Ime` 前缀，贴近 UI 业务命名 | `KeyboardPanel`, `KeyboardScreen`, `EditTextBridge`, `GestureFeedbackPanel`, `CandidateListPanel` |
...
| 组件 | 包路径 | 说明 |
|------|--------|------|
| `KeyboardPanel` | `integration` | 叠加模式完整输入法组件（候选栏 + 输入栏 + 工具栏 + 三层面板叠加：GestureInputPanel + GestureFeedbackPanel + KeyGridPanel，合并原 ThreeLayerKeyboardArea） |
...
叠加模式完整输入法组件，合并了原 `ThreeLayerKeyboardArea` 和 `InputScreen` 的职责。包含候选栏、输入栏、工具栏和三层面板叠加区域（GestureInputPanel / GestureFeedbackPanel / KeyGridPanel 直接叠加），是叠加布局模式下的完整输入法 UI 组件。

与 `KeyboardScreen`（全屏模式，第 5.4b 节）均为完整的输入法组件，包含相同的输入法功能（候选栏、输入栏、工具栏、键盘区域），二者只是形式和交互上存在差异：`KeyboardPanel` 是叠加模式，三层面板完全重叠，适合嵌入其他布局；`KeyboardScreen` 是全屏模式，手势输入面板与按键面板分离布局，占据全屏。

```kotlin
/**
 * 叠加模式完整输入法组件。
 *
 * 包含候选栏、输入栏、工具栏和三层面板叠加区域，
 * 构成完整的输入法 UI，无需外部再组合候选栏/输入栏/工具栏。
 * 合并了原 ThreeLayerKeyboardArea 和 InputScreen 的职责。
 *
 * 三层面板叠加区域：GestureInputPanel / GestureFeedbackPanel / KeyGridPanel
 * 直接叠加，无需中间包装层。
 * 底层按键渲染、中层反馈绘制、顶层手势拦截，三层面板完全重叠。
 * 这是与 Java 版本行为一致的默认模式。
 *
 * UI 库通过此组件向第三方应用提供叠加模式的完整输入法组件。
 * 第三方应用可直接将此组件嵌入自己的布局中。
 *
...
全屏模式完整输入法组件，手势输入面板与按键面板分离布局。包含候选栏、输入栏、工具栏和分离布局的键盘区域（手势输入面板占据屏幕下半区，按键面板在屏幕上半区浮动显示，反馈面板的多实例分别叠加在各区域），是全屏布局模式下的完整输入法 UI 组件（详见文档 150 第 3.2 节的 FullScreen 布局模式）。

与 `KeyboardPanel`（叠加模式，第 5.4 节）均为完整的输入法组件，包含相同的输入法功能（候选栏、输入栏、工具栏、键盘区域），二者只是形式和交互上存在差异：`KeyboardScreen` 是全屏模式，手势输入面板与按键面板分离布局，占据全屏；`KeyboardPanel` 是叠加模式，三层面板完全重叠，适合嵌入其他布局。
 ...
 *
 * 手势输入面板与按键面板分离布局：
 * - 屏幕下半区：手势输入面板（顶层）+ 反馈面板（底层，绘制触摸轨迹和手指指示器）
 * - 屏幕上半区：反馈面板（底层，绘制按键高亮和按键路径）+ 按键面板
 *
 * 输入面板的触摸点映射到按键面板的布局空间进行按键定位（详见文档 150 第 8.3 节）。
 *
 * 包含候选栏、输入栏和工具栏，构成完整的全屏输入法界面。
 *
 * @param engine IME 引擎实例
 * @param modifier 修饰符
 * @param inputPanelRatio 输入面板占屏幕高度的比例（默认 0.5）
 */
@Composable
fun KeyboardScreen(
    engine: ImeEngine,
    ...
    KeyboardTheme(themeType = state.config.ui.themeType) {
        Column(modifier = modifier.fillMaxSize()) {
            ...
            Box(
                modifier = Modifier
                    .fillMaxWidth()
                    .weight(1f - inputPanelRatio)
            ) {
                // 底层：反馈面板（按键侧：按键高亮、按键路径、X-Pad 路径高亮）
                GestureFeedbackPanel(
            ...
            Box(
                modifier = Modifier
                    .fillMaxWidth()
                    .weight(inputPanelRatio)
            ) {
                // 底层：反馈面板（输入侧：触摸轨迹、手指指示器）
                GestureFeedbackPanel(
...
fun ChatScreen() {
    ...
    Column {
        ...
        // 叠加模式：KeyboardPanel 已包含候选栏 + 输入栏 + 工具栏 + 三层面板叠加
...
fun FullscreenInputScreen() {
    ...
    val bridge = remember { InputConnectionBridge { currentInputConnection } }

    LaunchedEffect(engine) {
        engine.attachOutputBridge(bridge)
    }

    // 全屏模式：KeyboardScreen 已包含候选栏 + 输入栏 + 工具栏 + 三层面板分离布局
    KeyboardScreen(engine = engine)
}
```
...
|------|------------|---------------|----------|
| **Clipboard** | 不创建剪贴板监听器；`ImeOutput.CommitText` 不包含剪贴板粘贴；剪贴板提示弹窗不显示 | `Inputboard` 初始化时检查，若禁用则跳过剪贴板组件创建 | `ClipTipPopup` 不渲染 |
            ...
            setContent {
                ...
                KeyboardTheme(themeType = state.config.ui.themeType) {
                    Column {
                        CandidateListPanel(
                            state = state.candidates,
                            onCandidateSelected = { engine.handleIntent(ImeIntent.SelectCandidate(it)) },
                        )
                        InputListPanel(
                            state = state.inputList,
                            onGapTapped = { engine.handleIntent(ImeIntent.MoveCursorTo(it)) },
                        )
                        KeyboardPanel(engine = engine)
                    }
                }
...
KeyboardPanel (公开入口, :ime-ui, 叠加模式完整输入法组件)
  ...
  └── Toolbar                     ← 工具栏（依赖 Compose + engine.state）

KeyboardScreen (公开入口, :ime-ui, 全屏模式完整输入法组件)
  ├── CandidateListPanel          ← 候选栏（依赖 Compose + engine.state）
  ├── InputListPanel              ← 输入栏（依赖 Compose + engine.state）
  ├── GestureInputPanel           ← 手势拦截层（依赖 Compose + engine.state）
  ├── KeyGridPanel                ← 按键渲染层（依赖 Compose + engine.state）
  ├── GestureFeedbackPanel (×2)   ← 反馈绘制层（输入侧 + 按键侧，依赖 Compose + GestureFeedbackState）
  └── Toolbar                     ← 工具栏（依赖 Compose + engine.state）
```
...
第三方应用通过引擎库和 UI 库在应用内嵌入完整输入法，这是最常见的使用场景。`KeyboardPanel` 和 `KeyboardScreen` 均为完整的输入法组件，包含候选栏、输入栏、工具栏和键盘区域，二者只是形式和交互上存在差异：叠加模式使用 `KeyboardPanel`（三层面板叠加），全屏模式使用 `KeyboardScreen`（手势与按键分离）。
...
fun CustomKeyboardPanel() {
    ...
    Column {
        ...
        // 使用 UI 库的键盘面板（已包含候选栏 + 输入栏 + 工具栏 + 三层面板叠加）
        KeyboardPanel(
            engine = engine,
            modifier = Modifier.fillMaxWidth(),
        )
...
第三方应用可以只使用 UI 库中的部分组件，而非完整的 `KeyboardPanel` 或 `KeyboardScreen`。`KeyboardPanel` 和 `KeyboardScreen` 均为完整的输入法组件，内部已组合了候选栏、输入栏、工具栏和键盘区域。若需要自定义布局，可使用独立的子组件（CandidateListPanel、InputListPanel、KeyGridPanel、GestureInputPanel、GestureFeedbackPanel 等）自行组合：

```kotlin
@Composable
fun MinimalKeyboardPanel() {
...
| `ImeIntegratedActivity` | `ImeOutputBridge` + `KeyboardPanel`/`KeyboardScreen`（接入示例） | `:ime-engine` + `:ime-ui` | 不绑定 Activity，可在任意 Composable 中使用 |
...
| `KeyboardPanel` + 手势检测 | `KeyboardPanel`（叠加模式）/ `KeyboardScreen`（全屏模式） | `:ime-ui` | 三层分离，职责清晰；两者均为完整输入法组件（含候选栏/输入栏/工具栏），叠加/全屏两种布局模式 |
...
| `MainboardView` | `KeyboardPanel`（叠加）/ `KeyboardScreen`（全屏） | `:ime-ui` | 两者均为完整输入法组件（含候选栏/输入栏/工具栏），叠加/全屏两种布局模式 |
