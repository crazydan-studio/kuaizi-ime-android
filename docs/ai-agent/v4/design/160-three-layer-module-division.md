...
| 组件 | 包路径 | 说明 |
|------|--------|------|
| `KeyboardPanel` | `integration` | 完整输入法 UI 组件（合并原 InputScreen，三层面板 + 候选栏 + 输入栏 + 工具栏） |
...
完整输入法 UI 组件，合并了原 `InputScreen` 的职责。将三层面板、候选栏、输入栏、工具栏组合为一个完整的输入法界面，是 UI 库对第三方应用开放的完整输入法 UI 能力：

```kotlin
/**
 * 筷字输入法完整输入法 UI 组件。
 *
 * 合并了原 InputScreen 的职责，将三层面板（GestureInputPanel / GestureFeedbackPanel / KeyGridPanel）、
 * 候选栏、输入栏、工具栏组合为一个完整的输入法界面。
 *
 * UI 库通过此组件向第三方应用提供完整的带 UI 的输入法能力。
...
class IMEService : InputMethodService() {
    ...
    override fun onCreateInputView(): View {
        return ComposeView(this).apply {
            setContent { KeyboardPanel(viewModel = viewModel) }
        }
    }
