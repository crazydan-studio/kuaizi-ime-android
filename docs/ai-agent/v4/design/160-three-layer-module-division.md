...
| 组件 | 包路径 | 说明 |
|------|--------|------|
| `KeyboardPanel` | `integration` | 三层面板叠加模式键盘面板（GestureInputPanel + GestureFeedbackPanel + KeyGridPanel 直接叠加，合并原 ThreeLayerKeyboardArea） |
| `KeyboardScreen` | `integration` | 全屏模式键盘界面（手势输入面板与按键面板分离布局 + 候选栏 + 输入栏 + 工具栏） |
...
三层面板叠加模式键盘面板，合并了原 `ThreeLayerKeyboardArea` 和 `InputScreen` 的职责。将三层面板（GestureInputPanel / GestureFeedbackPanel / KeyGridPanel）直接叠加组合为键盘面板，是叠加布局模式下的键盘 UI 组件。

与 `KeyboardScreen`（全屏模式，第 5.4b 节）的区别：`KeyboardPanel` 是 Panel（面板），三层面板直接叠加，适合嵌入其他布局；`KeyboardScreen` 是 Screen（全屏界面），手势输入面板与按键面板分离布局，占据全屏。

```kotlin
/**
 * 三层面板叠加模式键盘面板。
 *
 * 将 GestureInputPanel / GestureFeedbackPanel / KeyGridPanel
 * 直接叠加组合为键盘面板，无需中间包装层。
 * 合并了原 ThreeLayerKeyboardArea 和 InputScreen 的职责。
 *
 * 适用于叠加布局模式：三层面板完全重叠，
 * 底层按键渲染、中层反馈绘制、顶层手势拦截。
 * 这是与 Java 版本行为一致的默认模式。
 *
 * UI 库通过此组件向第三方应用提供叠加模式的键盘面板。
 * 第三方应用可将此面板嵌入自己的布局中，与候选栏、输入栏、工具栏等组合使用。
 *
...
全屏模式键盘界面，手势输入面板与按键面板分离布局。在全屏模式下，手势输入面板占据屏幕下半区用于手势输入，按键面板在屏幕上半区浮动显示，反馈面板的多实例分别叠加在各区域（详见文档 150 第 3.2 节的 FullScreen 布局模式）。
...
fun ChatScreen() {
    ...
    Column {
        ...
        // 叠加模式：KeyboardPanel + 候选栏 + 输入栏 + 工具栏
...
fun FullscreenInputScreen() {
    ...
    // 全屏模式：KeyboardScreen 包含完整的分离布局 + 候选栏 + 输入栏 + 工具栏
...
KeyboardPanel (公开入口, :ime-ui, 叠加模式)
...
KeyboardScreen (公开入口, :ime-ui, 全屏模式)
  ├── GestureInputPanel            ← 手势拦截层（依赖 Compose + engine.state）
  ├── KeyGridPanel              ← 按键渲染层（依赖 Compose + engine.state）
...
第三方应用通过引擎库和 UI 库在应用内嵌入完整输入法，这是最常见的使用场景。叠加模式使用 `KeyboardPanel`（三层面板叠加）+ 候选栏 + 输入栏 + 工具栏组合，全屏模式使用 `KeyboardScreen`（手势与按键分离，内含候选栏/输入栏/工具栏）：
...
fun CustomKeyboardPanel() {
    ...
    Column {
        ...
        // 使用 UI 库的键盘面板（仅三层面板叠加，无候选/输入/工具栏）
        KeyboardPanel(
            engine = engine,
            modifier = Modifier.fillMaxWidth().height(280.dp),
        )
...
第三方应用可以只使用 UI 库中的部分组件，而非完整的 `KeyboardPanel` 或 `KeyboardScreen`：
...
| `KeyboardPanel` + 手势检测 | `KeyboardPanel`（叠加模式）/ `KeyboardScreen`（全屏模式） | `:ime-ui` | 三层分离，职责清晰；叠加/全屏两种布局模式 |
...
| `MainboardView` | `KeyboardPanel`（叠加）/ `KeyboardScreen`（全屏） | `:ime-ui` | 叠加/全屏两种布局模式 |
