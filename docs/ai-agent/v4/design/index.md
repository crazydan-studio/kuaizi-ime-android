...
| 170 | [模块 Class 组织结构](170-module-class-organization.md) | 三层模块的包结构与 class 组织统一设计：`:ime-engine`（api/bridge/domain/dict/input/state/logging）、`:ime-ui`（integration/panel/keyboard/candidate/input/editor/clipboard/favorite/practice/theme）、`:app`（顶级/settings/logging/guide）各包的 class 清单、类型、可见性、职责说明，以及 Java 版本 class 迁移对照 |
