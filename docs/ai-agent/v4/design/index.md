...
| 920 | [配置界面改进设计](920-config-ui-improvement.md) | 设置页面重构：场景化分组、肯定式命名、即时预览、条件显示、搜索、快捷切换弹窗、渐进式披露、保留「支持开发」捐赠入口 |
| 930 | [输入动作程序化设计](930-input-action-programming.md) | 输入动作脚本化与动画模拟：Release 可用的输入练习演示、字符序列→动作脚本编译、滑行/点击/X-Pad 动画、手指指示器与轨迹、操作录制（仅 debug）、自动化测试驱动 |
