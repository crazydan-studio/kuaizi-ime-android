...
| 160 | [IME 引擎库设计](160-ime-engine-library.md) | 核心 IME 引擎作为独立 `:ime-engine` 库模块：引擎与 UI 完全分离，配置通过 ImeEngineConfig 代码设置（不含持久化），DictProvider 接口支持替换内置 SQLite 实现，Feature 标记按需禁用收藏/剪贴板等功能；提供 ImeEngine（核心入口）、ImeOutput（编辑输出）、ImeEditText（便利组件）等公开 API |
