...
fun KeyboardArea(
    ...
    var keyPanelLayout by remember {
        mutableStateOf(KeyGridPanelLayoutInfo(emptyMap(), null, null))
    }
...
//  * 反馈面板的两个实例分别叠加在各区域：
//  * - 输入区域：GestureInputPanel（顶层）→ GestureFeedbackPanel（底层，触摸轨迹和手指指示器）
//  * - 按键区域：GestureFeedbackPanel（底层，按键高亮和按键路径）→ KeyGridPanel
...
| UI Compose 迁移（400） | `KeyboardPanel` 中的 `KeyboardArea` 替代原 `KeyboardArea`，内含三层面板结构；`KeyView` 移除 `combinedClickable` 和手势反馈逻辑 |
