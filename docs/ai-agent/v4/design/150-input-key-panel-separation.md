...
sealed class LayoutMode {
    ...
    //  * - 输入区域：InputPanel（顶层）→ FeedbackPanel（底层，绘制触摸轨迹和手指指示器）
    //  * - 按键区域：FeedbackPanel（底层，绘制按键高亮和按键路径）→ KeyPanel
    //  *
...
在全屏分离模式下，输入面板和按键面板不再重叠，反馈面板的多实例分别叠加在各自区域（输入面板始终在最上层拦截触摸）：
...
//  * 反馈面板的两个实例分别叠加在各区域：
//  * - 输入区域：InputPanel（顶层）→ FeedbackPanel（底层，触摸轨迹和手指指示器）
//  * - 按键区域：FeedbackPanel（底层，按键高亮和按键路径）→ KeyPanel
//  * 输入面板始终在各区域的最上层，确保触摸事件优先到达。
//  */
// @Composable
