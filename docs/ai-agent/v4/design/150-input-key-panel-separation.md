...
1. **输入面板是唯一的触摸事件接收者**，按键面板和反馈面板不处理触摸
2. **反馈面板是唯一的视觉反馈绘制者**，输入面板不绘制反馈，按键面板仅绘制按键的常规状态渲染（非手势触发的临时反馈）
...
三层面板的相对位置关系由 `LayoutMode` 定义，当前版本仅实现最基本的叠加模式：
...
sealed class LayoutMode {
    ...
    //  * 按键面板在屏幕上半区浮动显示。
    //  * 反馈面板的多实例分别叠加：
    //  * - 输入区域：一个 FeedbackPanel 叠加在输入面板上，绘制触摸轨迹
    //  * - 按键区域：一个 FeedbackPanel 叠加在按键面板上，绘制按键高亮
    //  *
 ...
 * 手势反馈状态，独立于任何面板。
 *
 * 由输入面板的手势事件驱动更新，供 FeedbackPanel 消费。
 ...
 *
 * 注意：手势视觉反馈由独立的 FeedbackPanel 绘制，
 * 输入面板不包含任何绘制逻辑。
 */
@Composable
fun InputPanel(
 ...
 *
 * 支持多实例：不同的 FeedbackPanel 实例可以绘制不同的反馈元素，
...
不同的布局模式下，反馈面板实例的配置不同。通过 `FeedbackPanelSet` 封装当前布局模式下的反馈面板组合：
...
sealed class FeedbackPanelSet {
     ...
     * 一个反馈面板即可覆盖所有反馈绘制。
     */
    data object OverlaySet : FeedbackPanelSet() {
 ...
 * 也不绘制手势视觉反馈（反馈由独立的 FeedbackPanel 负责）。
...
fun KeyboardArea(
    ...
    // 按键面板布局信息（供输入面板和反馈面板查询）
    var keyPanelLayout by remember {
        mutableStateOf(KeyPanelLayoutInfo(emptyMap(), null, null))
    }
...
在全屏分离模式下，输入面板和按键面板不再重叠，反馈面板的多实例分别叠加在各自区域：
...
//  * 全屏分离模式下的键盘区域。
//  *
//  * 输入面板在屏幕下半区，按键面板在屏幕上半区浮动。
//  * 反馈面板的两个实例分别叠加：
//  * - 输入区域：绘制触摸轨迹和手指指示器
//  * - 按键区域：绘制按键高亮和按键路径
//  */
// @Composable
// fun FullScreenKeyboardArea(
//     keyboardType: KeyboardType,
//     keyGrid: List<List<InputKey>>,
//     keyboardState: KeyboardState,
//     feedbackState: GestureFeedbackState,
...
//     modifier: Modifier = Modifier,
// ) {
//     var keyPanelLayout by remember { mutableStateOf(...) }
//     val config = layoutMode as LayoutMode.FullScreen
//
...
//             modifier = Modifier
//                 .fillMaxWidth()
//                 .weight(config.inputPanelRatio)
//                 .offset(y = config.keyPanelFloatingOffset.y),
//         ) {
//             // 按键面板
//             KeyPanel(...)
//
//             // 按键侧反馈面板（按键高亮、按键路径、X-Pad 高亮）
//             FeedbackPanel(
//                 elements = FeedbackPanelSet.FullScreenSet.keySideElements,
//                 feedbackState = feedbackState,
//                 keyPanelLayout = keyPanelLayout,
//             )
//         }
...
//             modifier = Modifier
//                 .fillMaxWidth()
//                 .weight(1f - config.inputPanelRatio),
//         ) {
//             // 输入面板
//             InputPanel(
//                 keyPanelLayout = keyPanelLayout,
...
//             // 输入侧反馈面板（触摸轨迹、手指指示器）
//             FeedbackPanel(
//                 elements = FeedbackPanelSet.FullScreenSet.inputSideElements,
//                 feedbackState = feedbackState,
//                 keyPanelLayout = keyPanelLayout,
//             )
//         }
...
文档 930 的 `KeyPositionResolver` 与本文档的 `KeyPanelLayoutInfo` 和 `GestureFeedbackState` 协作：

### 9.1 KeyPanelPositionResolver
...
这样，输入动作程序化系统与三层面板分离架构共享同一套 `GestureFeedbackState` 和 `FeedbackPanel`，无需维护独立的视觉反馈逻辑。
...
| 区域高亮 | XPadView.onDraw()（状态驱动） | FeedbackPanel.drawXPadPathHighlight() |
| 触摸轨迹 | 无（X-Pad 不绘制轨迹） | FeedbackPanel.drawTouchTrail() |
 ...
 * 不绘制手势反馈（如区域高亮、路径），
 * 不处理触摸事件。
 * 手势高亮由 FeedbackPanel 的 XPadPathHighlight 元素绘制，
 * 触摸检测由 InputPanel 的 handleXPadGesture 负责。
 */
@Composable
fun XPadKeyContent(
    key: InputKey.XPad,
...
X-Pad 模式的手势高亮由反馈面板负责，与标准按键模式的反馈绘制复用同一个 `FeedbackPanel` 实例（叠加模式下）：
...
1. **手势 → 逻辑**：`InputPanel → InputGesture → ViewModel → IMEIntent → reduce() → IMEState`
2. **手势 → 反馈**：`InputPanel → GestureFeedbackState → FeedbackPanel → Canvas 绘制`
3. **状态 → 渲染**：`IMEState → KeyPanel → Compose 重组`
4. **程序化输入 → 反馈**：`InputActionPlayer → GestureFeedbackState → FeedbackPanel → Canvas 绘制`
...
叠加模式是基础，全屏输入模式是重要扩展方向。在全屏输入模式下，输入面板和按键面板不再重叠，反馈面板的多实例分别叠加：
...
2. 新增 `FeedbackPanelSet.FullScreenSet` 配置
3. 修改 `KeyboardArea` 的组合逻辑（不再叠加，改为上下布局）
...
后续版本可在配置系统中添加反馈样式选项，如轨迹颜色、轨迹宽度、高亮透明度、手指指示器大小等。这些配置项通过 `GestureFeedbackState` 传递给 `FeedbackPanel`，不影响输入面板和按键面板。
...
| `KeyboardView`（按键状态动画） | `KeyPanel`（KeyboardState 驱动） | 持续性状态渲染保留在按键面板 |
| `RecyclerViewGestureTrailer` | `FeedbackPanel.drawTouchTrail()` | 轨迹绘制从 ItemDecoration 移至独立反馈面板 |
| `KeyboardViewKeyAnimator`（临时高亮） | `FeedbackPanel.drawKeyHighlights()` | 临时性手势高亮移至反馈面板 |
| `XPadView.onTouchEvent()` | `InputPanel.handleXPadGesture()` | X-Pad 手势由输入面板统一处理 |
| `XPadView.onDraw()`（静态布局） | `KeyPanel.XPadKeyContent` | X-Pad 静态绘制保留在按键面板 |
| `XPadView.onDraw()`（区域高亮） | `FeedbackPanel.drawXPadPathHighlight()` | X-Pad 手势高亮移至反馈面板 |
...
| UI Compose 迁移（400） | `IMEScreen` 中的 `KeyboardArea` 替代原 `KeyboardArea`，内含三层面板结构；`KeyView` 移除 `combinedClickable` 和手势反馈逻辑 |
| X-Pad 重构（700） | X-Pad 的手势检测从 `XPadView` 移至输入面板，静态绘制保留在按键面板，区域高亮移至反馈面板 |
| 输入动作程序化（930） | `KeyPanelPositionResolver` 复用 `KeyPanelLayoutInfo`，共享按键位置查询；`InputActionPlayer` 通过 `GestureFeedbackState` 驱动反馈面板，与真实手势共享反馈基础设施 |
| 配置系统（500） | `LayoutMode` 和 `FeedbackPanelSet` 可作为配置项持久化（当前仅有 Overlay）；反馈样式配置（颜色、宽度等）可扩展 |
