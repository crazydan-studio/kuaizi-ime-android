...
sealed class LayoutMode {
    ...
    //  * - 输入区域：InputPanel（顶层）→ GestureFeedbackPanel（底层，绘制触摸轨迹和手指指示器）
    //  * - 按键区域：GestureFeedbackPanel（底层，绘制按键高亮和按键路径）→ KeyPanel
    //  *
...
//  * 反馈面板的两个实例分别叠加在各区域：
//  * - 输入区域：InputPanel（顶层）→ GestureFeedbackPanel（底层，触摸轨迹和手指指示器）
//  * - 按键区域：GestureFeedbackPanel（底层，按键高亮和按键路径）→ KeyPanel
...
2. **手势 → 反馈**：`InputPanel → GestureFeedbackState → GestureFeedbackPanel → Canvas 绘制`
