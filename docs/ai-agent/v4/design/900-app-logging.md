...
|-------------|---------|---------|
| `Logger.getLogger()` | `ImeLog.logger()` | 支持所有构建类型，不再 release 返回 noop |
| `Logger.debug/info/warn/error` | `ImeLogger.verbose/debug/info/warn/error` | 新增 VERBOSE 级别，inline lambda 延迟求值 |
| `Logger.beginTreeLog/endTreeLog` | `ImeLogger.tree { }` | 作用域函数替代手动 begin/end，避免遗漏 end |
