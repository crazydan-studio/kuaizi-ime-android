...
> **注意**：三层库架构的详细设计见 [030-三层模块划分](./030-module-division.md)。v4 采用三层库架构：引擎库（`:ime-engine`，逻辑层与UI/应用分离，Domain Layer + Data Layer）、UI 库（`:ime-ui`，Compose 缺省 UI + ViewModel Layer）、应用模块（`:app`，Platform Layer + 配置持久化 + 设置页面）。第三方应用可以引入 `:ime-engine` + `:ime-ui` 获得完整的输入法能力与缺省 UI（含 KeyboardViewModel），也可以仅引入 `:ime-engine` 自行实现 UI。
