...
- **应用模块 `:app`**：系统 IME 服务壳、设置页面、输入练习 UI（ExerciseScreen）、配置持久化，是库的官方消费者（注：InputActionPlayerPanel 属于 `:ime-ui` 模块）
...
- **KeyboardInputMode**：输入交互范式枚举（XPad/HexGrid/RectGrid/MultiZone），决定按键的几何排列和交互方式
