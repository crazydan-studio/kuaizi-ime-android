...
v4 版本使用 DataStore + Flow 实现类型安全的配置管理和响应式更新。`ImeConfig` 包含 `EngineConfig`（引擎配置）和 `UiConfig`（UI 配置）两个嵌套 data class，引擎与 UI 配置在数据结构上明确隔离。`:app` 模块的 `ConfigRepository` 直接持久化和恢复 `ImeConfig`。
...
> `ImeConfig` 定义在 `:ime-engine` 库中，包含 `EngineConfig` 和 `UiConfig`。完整的 ImeConfig 定义见文档 160 第 4.2 节。

---

## 2. ConfigRepository（DataStore 实现）
