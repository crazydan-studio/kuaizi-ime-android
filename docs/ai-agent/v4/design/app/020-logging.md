...
应用层不修改引擎的日志核心逻辑，仅通过 `ImeLog.init()` 注入平台特有的配置和使用引擎提供的工具类（`LogcatWriter`、`CrashInterceptor`），通过 `ImeLog.updateLevel()` 和 `LogStorage.updateDir()` 响应用户的运行时配置变更。这种分层确保了引擎库与 UI / 应用层的解耦，同时为 Android 应用提供完整的日志能力。
