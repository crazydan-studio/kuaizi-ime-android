...
v4 版本新增输入动作程序化能力，即通过指定待输入的字符序列，自动以动画模拟滑行或点击等方式完成输入过程。该功能主要用于输入操作动画演示（如教学引导、功能展示），也可用于自动化测试。核心思路是：将用户真实的手指操作抽象为可序列化的「动作脚本」，由播放引擎按脚本驱动键盘状态机和 UI 动画，产生与真人操作一致的视觉效果。
...
| 场景 | 说明 | 输入方式 |
|------|------|----------|
| 新手引导 | 首次使用时演示滑行输入「你好」的完整过程 | 滑行动画 |
| 功能展示 | 在应用商店截图/录屏中展示 X-Pad 连续输入 | 滑行 + X-Pad 动画 |
| 键盘布局教学 | 展示特定拼音的滑行路径（如输入「zhuang」的完整路径） | 滑行动画 |
| 候选选择演示 | 展示如何选择候选字、翻页查看更多候选 | 点击动画 |
| 自动化测试 | 按脚本自动操作键盘，验证状态机转换和输出正确性 | 即时执行（无动画） |
| 操作回放 | 记录并回放用户的真实操作路径 | 真实路径回放 |
...
| 与 UI 测试工具隔离 | 仅 debug 构建包含动画播放引擎，release 构建自动移除 |
...
除了从字符序列编译脚本，还可以录制用户的真实操作路径生成脚本：
...
录制功能通过 UI 测试工具栏（文档 910）的浮动按钮触发：
...
输入动作程序化的代码分为两部分：核心模型（main 源集）和动画/UI（debug 源集）：

| 层 | 源集 | 内容 | Release 包含 |
|----|------|------|-------------|
| 模型层 | `main` | `InputAction`、`ActionScript`、`InputMethod` 定义 | ✅ 数据类极小，无逻辑 |
| 编译层 | `main` | `ActionScriptCompiler`（字符→动作） | ✅ 可用于自动化测试 |
| 播放层 | `debug` | `InputActionPlayer`、`FingerOverlay`、`SwipeTrailOverlay` | ❌ |
| UI 层 | `debug` | `ActionPlayerPanel`、`RecordButton`、预置脚本 | ❌ |
| 测试驱动 | `test` | `ActionTestDriver` | ❌（test 依赖） |

### 9.2 Release 构建影响

Release 构建中：
- `InputAction` 和 `ActionScript` 数据类存在但不会被引用（ProGuard 可移除）
- 所有动画和 UI 组件不在 APK 中
- 预置脚本不在 APK 中（放在 `debug` 源集的 assets 中）
...
| UI 测试工具（910） | 录制/播放入口集成到 UI 测试浮动工具栏 |
| 配置系统（500） | 播放速度等参数可作为配置项持久化 |
