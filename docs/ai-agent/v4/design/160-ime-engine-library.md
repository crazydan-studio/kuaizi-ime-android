...
| `:ime-ui` | 不使用 `Ime` 前缀，贴近 UI 业务命名 | `KeyboardPanel`, `TextEditorBridge`, `GestureFeedbackPanel`, `CandidatePanel` |
...
2. **ui 模块**：组件命名贴近其 UI 职能，使用业务语义化的名称：面板类以 `Panel` 为后缀（如 `GestureInputPanel`、`KeyGridPanel`、`CandidatePanel`、`GestureFeedbackPanel`）、视图类以 `View` 为后缀（如 `XPadView`）、桥梁类以 `Bridge` 为后缀（如 `TextEditorBridge`）。主题系统使用 `Keyboard` 前缀（如 `KeyboardColors`、`KeyboardTheme`），与 Material3 的 `MaterialTheme` 等系统命名区分
...
> **注意**：编辑器桥接组件在 `:ime-engine` 的 `api/` 包中定义（ImeOutputBridge、BaseImeOutputBridge），具体实现分别在 `:app`（InputConnectionBridge）和 `:ime-ui`（TextEditorBridge）中
...
:ime-ui/          → TextEditorBridge（替代 EditorField）
...
#### 5.3.2 TextEditorBridge（EditText 类型默认实现）
...
fun ChatScreen() {
    ...
    val bridge = remember { TextEditorBridge { currentEditText } }
...
KeyboardPanel (公开入口, :ime-ui)
  ...
  ├── TextEditorBridge          ← 编辑器桥梁（依赖 BaseImeOutputBridge + EditText）
...
| 收藏操作 | `Favorites` | 收藏当前选中文本 | `EditAction(FAVORITE)` |
...
| `ImeSupportEditText` | `ImeOutputBridge` + `TextEditorBridge` | `:ime-engine` + `:ime-ui` | 自动消费 ImeOutput |
