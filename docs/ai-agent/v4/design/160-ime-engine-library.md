...
│   │           ├── panel/       ← 三层面板（InputPanel, KeyPanel, GestureFeedbackPanel）
...
│   │           └── integration/ ← 集成组件（KeyboardView, InputHostView）
│   │
...
| `:ime-ui` | 不使用 `Ime` 前缀，贴近 UI 业务命名 | `KeyboardView`, `EditorField`, `GestureFeedbackPanel`, `CandidateBar` |
...
2. **ui 模块**：组件命名贴近其 UI 职能，使用业务语义化的名称：面板类以 `Panel` 为后缀（如 `GestureFeedbackPanel`）、栏类以 `Bar` 为后缀（如 `CandidateBar`）、视图类以 `View` 为后缀（如 `KeyboardView`、`XPadView`）、编辑器类直接使用职能名称（如 `EditorField`、`EditorHost`）。主题系统使用 `Keyboard` 前缀（如 `KeyboardColors`、`KeyboardTheme`），与 Material3 的 `MaterialTheme` 等系统命名区分
3. **app 模块**：Android 系统服务类沿用平台命名惯例（如 `IMEService`），配置类使用职能名称（如 `ConfigRepository`），桥接类使用目标对象命名（如 `InputConnectionBridge`）。页面以 `Screen` 为后缀（如 `SettingsScreen`、`GuideScreen`）
...
| 组件 | 包路径 | 说明 |
|------|--------|------|
| `KeyboardView` | `integration` | 完整键盘组件（三层面板 + 候选栏 + 输入栏 + 工具栏） |
| `InputHostView` | `integration` | EditorField + KeyboardView 的完整输入方案 |
 ...
 * 筷字输入法完整键盘组件。
 *
 * 将三层面板（InputPanel / GestureFeedbackPanel / KeyPanel）、
...
最顶层的集成组件，将 EditorField 和 KeyboardView 组合为完整的输入方案，对应 Java 版本中 `ImeIntegratedActivity` 提供的能力，但更加灵活：

```kotlin
/**
 * 完整输入方案集成组件。
 *
 * 将 EditorField 和 KeyboardView 组合为一个完整的输入法方案。
...
|------|------------|---------------|----------|
| **Clipboard** | 不创建剪贴板监听器；`ImeOutput.CommitText` 不包含剪贴板粘贴；剪贴板提示弹窗不显示 | `Inputboard` 初始化时检查，若禁用则跳过剪贴板组件创建 | `ClipTipPopup` 不渲染；`KeyboardView` 隐藏剪贴板相关 UI |
...
| 职责 | `:ime-ui` 库 | `:app` 模块 |
|------|-------------|------------|
| 三层面板（InputPanel / KeyPanel / GestureFeedbackPanel） | ✅ | ❌ |
| 候选栏、输入栏 | ✅ | ❌ |
| EditorField、EditorHost | ✅ | ❌ |
| KeyboardView、InputHostView | ✅ | ❌ |
...
第三方应用可以只使用 UI 库中的部分组件，而非完整的 `KeyboardView`：
...
| 面板分离（150） | 三层面板（InputPanel / KeyPanel / GestureFeedbackPanel）在 `:ime-ui` 库中实现，引擎不包含面板代码 |
...
| `KeyboardView` + 手势检测 | `InputPanel` + `KeyPanel` + `GestureFeedbackPanel` | `:ime-ui` | 三层分离，职责清晰 |
...
| `MainboardView` | `KeyboardView` | `:ime-ui` | 一站式键盘组件 |
