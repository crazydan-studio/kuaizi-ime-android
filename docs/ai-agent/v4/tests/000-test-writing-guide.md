...
计划文档（`plans/`）中的验收标准应关联到具体的测试用例编号。验收员通过执行测试来判定验收是否通过，流程如下：
...
测试文档以三位数字开头，与对应的设计文档编号一致，后缀为 `-tests.md`：

```
100-keyboard-state-machine-tests.md  ← 对应 design/100-keyboard-state-machine.md
160-ime-engine-api-tests.md          ← 对应 design/160-ime-engine-library.md
200-input-list-tests.md              ← 对应 design/200-input-list-redesign.md
```
...
| 编号 | 说明 |
|------|------|
| TC-100-001 | 键盘状态机测试文档的第 1 个用例 |
| TC-160-015 | 引擎库 API 测试文档的第 15 个用例 |
| TC-500-003 | 配置系统测试文档的第 3 个用例 |
...
```kotlin
@Test
fun `should transition to InputWaiting when KeyPressed intent received in Idle state`() {
    // ...
}
...
fun `should commit text when CommitInput intent processed with non-empty input list`() {
    ...
    val engine = ImeEngine.create(
        config = ImeConfig(),
        dictProvider = InMemoryDictProvider(testPinyinData),
    )
    engine.handleIntent(ImeIntent.KeyPressed(key = InputKey.Char('n'), gesture = KeyGesture.Tap))
    engine.handleIntent(ImeIntent.KeyPressed(key = InputKey.Char('i'), gesture = KeyGesture.Tap))
...
- 不依赖共享的可变状态
- 需要外部依赖时使用 Mock 或内存实现（如 `InMemoryDictProvider`），不依赖真实数据库或文件系统

### 6.4 Mock 使用原则

- **优先使用真实实现**：对于纯逻辑组件（状态机、InputList、ImeConfig），直接使用真实实现而非 Mock
- **Mock 仅用于外部依赖**：数据库、文件系统、系统服务等无法在单元测试中使用的依赖才使用 Mock
- **使用接口而非 Mock 库**：对于字典等可替换组件，优先使用 `InMemoryDictProvider` 等测试专用实现，而非 Mockito 等 Mock 框架
- **不 Mock 被测类自身**：被测类的内部方法不应被 Mock，否则测试失去意义

```kotlin
// ✅ 正确：使用内存实现替代数据库
val dictProvider = InMemoryDictProvider(mapOf("ni" to listOf(testWord)))
...
| 计划文档（`plans/`） | 计划的验收标准通过测试用例的执行结果来验证 |
